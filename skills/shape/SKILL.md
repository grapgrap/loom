...
흐름이 본질이 아닌 도메인(구조가 중심인 단순 도메인)은 경계만 긋고 닫는다 — 모든 도메인이 flow를 갖는 것은 아니다. 단계는 도메인 경계를 넘는 수준에 머물고, 한 도메인 내부의 구현으로 내려가지 않는다.
...
도메인들을 가로지르는 흐름이 식별되면 `.loom/flows/`에 파일을 생성한다. 기존 흐름이 변경되면 해당 파일을 수정한다.

flow에는 목적과 단계를 기술한다. 각 단계는 도메인 경계를 넘는 수준이며, 한 도메인 내부의 구현은 담지 않는다. 변이는 별도 항목이 아니라 하위 흐름으로 표현한다.

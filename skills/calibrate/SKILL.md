...
- **decision과의 괴리**: decision의 결론이 concept의 현재 상태와 모순되는가 — concept이 항상 현재 진실이므로, 모순되는 decision은 사실상 폐기된 것이다
- **원칙과의 괴리**: concept의 설계 방향이 원칙의 **적용** 시나리오와 부합하는가, **테스트** 질문을 통과하는가

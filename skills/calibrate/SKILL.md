...
name: calibrate
description: >-
  concept과 코드의 누적 괴리를 감사하고 리포트한다. 문서와 코드 사이에 괴리가 쌓였다고 느낄 때
  수동으로 호출한다. "calibrate", "교정", "concept 감사" 같은 맥락에서 트리거된다.
---

# Calibrate

concept과 코드의 정합성을 감사한다.

review가 세션 단위의 검증이라면, calibrate는 축적된 괴리의 감사다. 발견을 리포트할 뿐 직접 교정하지 않으며, 교정 방향은 사용자가 shape 재진입으로 판단한다.
...
누적 감사의 기준에는 rule·convention이 포함된다. 인지한 룰 카탈로그를 도구 선택 절차로 활용한다:
